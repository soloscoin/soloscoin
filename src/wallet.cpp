...
// soloscoin: optional setting to unlock wallet for block minting only;
...
// soloscoin: total coins staked (non-spendable until maturity)
...
bool CWallet::SelectCoinsMinConf(int64 nTargetValue, unsigned int nSpendTime, int nConfMine, int nConfTheirs, vector<COutput> vCoins, set<pair<const CWalletTx*,unsigned int> >& setCoinsRet, int64& nValueRet) const
{
    ...
    BOOST_FOREACH(COutput output, vCoins)
    {
        ...
        if (pcoin->nTime > nSpendTime)
            continue;  // soloscoin: timestamp must not exceed spend time
...

    {
        ...
        CTxDB txdb("r");
        {
            ...
            while(true)
            {
                ...
                // soloscoin: sub-cent change is moved to fee
...
// soloscoin: create coin stake transaction
...
// soloscoin: check 'spent' consistency between wallet and txindex
// soloscoin: fix wallet spent state according to txindex
...
// soloscoin: disable transaction (only for coinstake)
